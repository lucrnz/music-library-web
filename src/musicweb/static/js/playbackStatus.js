 ...
 * @typedef {{ id: string, label?: string, kind?: string, bitrate_kbps?: number, bit_depth?: number, sample_rate?: number }} ProfileMeta
 ...
 */
export function formatPrimaryStatus(state, catalog = []) {
 ...
 * @param {ProfileMeta[]} [catalog]
 */
export function formatStatusAriaLabel(state, catalog = []) {
  const face = formatPrimaryStatus(state, catalog);
  if (!face.interactive) return face.text;
  if (state.playSource === "unavailable") {
 ...
 */
export function buildPlaybackDetailsRows(state, catalog = []) {
  const playSource = state?.playSource || "none";
