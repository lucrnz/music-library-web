...
from musicweb.routes.deps import scanner
...
def scan_start(request: Request, payload: ScanRequest) -> dict:
    sc = scanner(request)
    started = sc.start(payload.mode)
    if not started:
        raise HTTPException(status_code=409, detail="Scan already running")
    return {"started": True, "mode": payload.mode, **sc.status()}


@router.post("/library/scan/cancel")
def scan_cancel(request: Request) -> dict:
    sc = scanner(request)
    ok = sc.request_cancel()
    return {"canceling": ok, **sc.status()}
