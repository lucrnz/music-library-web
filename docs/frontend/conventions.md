...
- **Copy.** All clipboard writes go through `copyText` in `clipboard.ts` (toasts “Copied” / “Could not copy”). Empty values are omitted from menus, not copied as blank. Settings diagnostic IDs use the same helper.
