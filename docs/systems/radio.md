...
`radio/runtime.ts` owns the socket and reconnect. `radio/session.ts` owns `onFaceOrTrack` and `loadCurrent`. `loadCurrent` calls `resolvePlaySource` (not `resolvePlayIntent`) with `offline: false` while the tuner socket is up, `enabled: downloads.enabled`, and `activeStreamCodec` = `source` when the official track is lossy, else the tuner Streaming profile. A failed local blob is `markTrackBroken` then reminted to `/api/stream` on the same generation. Connect on Radio tab enter; do not disconnect on leave. Socket stays up for the Radio tab or chrome `stopped` | `tuning` | `tuned` (one `socketRequired`). `connected` tracks socket open. Audio is a radio-owned `HTMLAudioElement` that implements `PlaybackSink` (`kind: "htmlAudio"`; shared `htmlElement` helper; not the on-demand queue sink). `radio.ts` and `NowPlayingView` do not import `player.ts`. Tune-in calls `become("radio")`; a library/queue play calls `become("queue")`. Radio watches `player.volume` from `playerState.ts` and `settings.playbackPolicy` (re-resolves while `tuning` / `tuned`). Chrome is `inactive | stopped | tuning | tuned` (tab-open-without-tune-in is `inactive` + `tabOpen`). Load generation in `session.ts` guards overlapping loads; the station-face handler is the only `loadCurrent` driver. The room is `NowPlayingView` via a thin radio wrapper (`RadioNowPlaying`), not a parallel now-playing tree. Title, then `Artist — album`. Transport is Tune in/out only — no shuffle, prev, next, repeat, or play/pause. Seek is filled and not interactive. A `pause` while the element has `ended` is ignored; `ended` is never Tune-out (the station clock owns advance). Lyrics overlay + toggle, `seekable=false`; radio lyrics open state is local to the wrapper. The room injects `PlaybackStatusLine` only while chrome is `tuned` (`radioPlayState()`: `session: "radio"`, real `playSource` + local catalog profile when downloaded, or tuner profile when streaming; lossy `playProfileId` stays null so the line uses source-format fields). Exclusive snap is ignored because session is radio. When chrome is not `tuned`, the room keeps an empty `.np-status-wrap` so extras do not jump. Compact and mini still have no codec line. After a successful tuned load, `session.ts` starts the household listen cycle (`origin: radio`, `play_source` = resolved delivery, profile = resolved tag including `source` when lossy). Time/ended samples run only while `tuned`. Tune out, leave radio, catch-up / skip-pending, a new `loadCurrent`, and failed tune discard the cycle. See `docs/systems/playback-stats.md`.
