...
- Socket, load generation, face machine (`onFaceOrTrack`), reconnect, radio Media Session: `frontend/src/radio/runtime.ts`
...
`radio/runtime.ts` owns the socket, `onFaceOrTrack`, and reconnect. Connect on Radio tab enter; do not disconnect on leave. Socket stays up for the Radio tab or chrome `stopped` | `tuning` | `tuned` (one `socketRequired`). `connected` tracks socket open. Audio is a radio-owned `HTMLAudioElement` (shared `htmlElement` helper; not the on-demand `PlaybackSink`). `radio.ts` and `NowPlayingView` do not import `player.ts`. Tune-in calls `become("radio")`; a library/queue play calls `become("queue")`. Radio watches `player.volume` from `playerState.ts`. Chrome is `inactive | stopped | tuning | tuned` (tab-open-without-tune-in is `inactive` + `tabOpen`). Load generation in `runtime.ts` guards overlapping loads; the station-face handler is the only `loadCurrent` driver. The room is `NowPlayingView` via a thin radio wrapper (`RadioNowPlaying`), not a parallel now-playing tree. Title, then `Artist — album`. Transport is Tune in/out only — no shuffle, prev, next, repeat, or play/pause. Seek is filled and not interactive. A `pause` while the element has `ended` is ignored; `ended` is never Tune-out (the station clock owns advance). Lyrics overlay + toggle, `seekable=false`; radio lyrics open state is local to the wrapper. The room injects `PlaybackStatusLine` only while chrome is `tuned` (`radioPlayState()`: `session: "radio"`, `playSource: "streaming"` + tuner profile, or lossy source fields). Exclusive snap is ignored because session is radio. When chrome is not `tuned`, the room keeps an empty `.np-status-wrap` so extras do not jump. Compact and mini still have no codec line. Radio does not write listen-stat events.
