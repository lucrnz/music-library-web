...
Resolution is decision-first: load catalog record when downloads are enabled, apply **playback policy**, open a local blob only if local wins, otherwise stream when the server is reachable. Play-source “online” means `canUseRemoteMedia()` in `connectivity.ts`: confirmed + `state === "online"`. It does not read `navigator.onLine`. Reachability truth lives in [connectivity.md](connectivity.md). Until confirmation, a playable download wins. When `canUseRemoteMedia()` is false, resolve uses the offline path: a playable download still wins. A `playIndex` of a row that would be `offline_no_local` probes the stream once (`probeRemote` in `playIntent.ts` / `resolve.ts`); a failed probe can still show `offline_no_local`. Do not force `prefer_stream` onto a playable local file. After a downloaded blob fails to play, stream fallback uses that same helper; unconfirmed or unreachable is `broken`, not `/api/stream`. A failed stream load does not fall back to OPFS while the session is confirmed reachable.
...
Until the hold completes, an unintentional pause, retryable error, soft reject (`NotAllowedError` / `AbortError`), load timeout, or `ended` far from duration stays on the current row and retries forever from last heard position (`currentTime` when it has moved, else the seek this `playIndex` already intended). Retry chrome is silent: no toast, `player.loadPending` stays true so Play shows “Loading stream…”. The rejoin clock is the shared 1 s → 8 s backoff (`createRejoinClock` in `rejoinClock.ts`). `kick()` (connectivity recovery, including radio) waits at least 250ms before the first attempt; it does not change the 1 s → 8 s table or the 8 s hold. After the hold elapses, pause is ordinary pause and the rejoin clock is not armed.
...
The **expanded** now-playing cover (mobile sheet, desktop panel) can 3D-flip to the album-artist photo. Offline: a downloaded artist **full** file or VA flags (`getLocalArtistFlip` in `art.ts`) work while the session is unreachable. Online / undownloaded: `GET /api/artists/{id}` plus remote `size=full` still requires the server. No flip from thumb-only. The peek resets on track change, collapse, or unmount. The lyrics overlay blocks the flip and does not change the face. Mini and compact-bar covers stay open-targets (expand now-playing); they do not flip. Helper: `frontend/src/components/player/coverFlip.ts`.
