...
- Session handoff: `frontend/src/playback/onDemandControl.ts` (`claimOnDemand`; tune-in uses `suspendMediaSession` then `stopOnDemandSinks`)
- On-demand teardown: `frontend/src/playback/teardown.ts` (`needsCompanionStop`)
