...
When exclusive audio is **enabled** on an installed Mac PWA, the Streaming picker is hidden. Downloads quality and **When a download exists** still apply: exclusive may play a companion-disk (or leftover OPFS HTML) download, or stream an exclusive FLAC tag (lossy: `source` into mpv). `exclusive_lossy` is only the case where there is no local file and no source URL. See `docs/systems/exclusive-audio.md`.
