...
}>(), { active: false, libraryReachable: true });
const statusText = ref("—");
    ...
    async function refreshScanStatus() {
      ...
      try {
        ...
        statusText.value = `Scan status unavailable: ${err instanceof Error ? err.message : String(err)}`;
      }
    }

    async function startScan(mode: string) {
      if (!props.libraryReachable) return;
      try {
        ...
        console.error(err);
        statusText.value = `Could not start scan: ${err instanceof Error ? err.message : String(err)}`;
      }
    }
